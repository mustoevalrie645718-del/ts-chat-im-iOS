...
class GroupSetupLogic extends GetxController {
  ...
  bool get canLookMembersInfo =>
      groupInfo.value.lookMemberInfo != 1 || myGroupMembersInfo.value.roleLevel != GroupRoleLevel.member;
  ...
  addMember() async {
    ...
    if (list is List<String>) {
      try {
        ...
        Logger.print('Failed to invite members: $e');
      }
      getGroupMembers();
    }
  }
  ...
  removeMember() async {
    ...
    if (list is List<GroupMembersInfo>) {
      await showRemoveMembersDialog(
          context: Get.context!,
          members: list,
          kickMemberAndDeleteMessages: _kickMemberAndDeleteMessages,
          kickMembers: _kickMembers);
      getGroupMembers();
    }
  }

  Future<void> showRemoveMembersDialog({
    required BuildContext context,
    required List<GroupMembersInfo> members,
    required Future Function(List<String> membersID) kickMemberAndDeleteMessages,
    required Future Function(List<String> membersID) kickMembers,
  }) async {
    bool deleteForOther = false;

    final isMultiple = members.length > 1;
    final dynamic result = await Get.dialog<dynamic>(
      CustomDialog(
        title: isMultiple ? StrRes.removeMembersHint : StrRes.removeMemberHint,
        alignment: Alignment.centerLeft,
        showCheckbox: false,
        checkboxText: isMultiple ? StrRes.deleteMembersMessageHint : StrRes.deleteMemberMessageHint,
        initialCheckboxValue: deleteForOther,
      ),
    );

    final membersID = members.map((message) => message.userID).where((id) => id != null).cast<String>().toList();

    if (membersID.isEmpty) {
      IMViews.showToast('No valid members to delete messages');
    ...
    if (result != null) {
      ...
      if (deleteForOther) {
        await kickMemberAndDeleteMessages(
          membersID,
        );
      } else {
        await kickMembers(membersID);
      }
    }
  }

  Future _kickMemberAndDeleteMessages(List<String> membersID) async {
    try {
      await LoadingView.singleton.wrap(
        asyncFunction: () async {
          await LoadingView.singleton.wrap(
      ...
      Logger.print('Failed to delete messages: $e');
      IMViews.showToast('Failed to delete messages: $e');
    }
  }

  Future _kickMembers(List<String> membersID) async {
    try {
      await LoadingView.singleton.wrap(
        asyncFunction: () => OpenIM.iMManager.groupManager.kickGroupMember(
          groupID: groupInfo.value.groupID,
      ...
      Logger.print('Failed to kick members: $e');
      IMViews.showToast('Failed to kick members: $e');
    }
  }

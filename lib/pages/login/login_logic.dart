...
class LoginLogic extends GetxController with GetTickerProviderStateMixin {
  ...
  final pushController = Get.find<PushController>();
  ...
  // final hadRead = false.obs;//卡卡西 新增是否阅读
  TabController? _tabController;
  TabController get tabController {
    if (_tabController == null) {
      _tabController = TabController(length: 1, vsync: this);
      _tabController!.index = loginType.value.rawValue;
    }
  ...
  void onReady() {
    super.onReady();
    getPackageInfo();
    Get.find<AppController>().autoCheckVersionUpgrade();
  }
  ...
  login() {
    // if(!hadRead.value){
    //   ToastUtil.showMyToast("请阅读并同意用户协议及隐私政策");
    //   return;
    // }
  ...
  Future<bool> _login() async {
    try {
      ...
      final data = await Apis.login(
        ...
        email: email,
        password: isPasswordLogin.value || loginType.value == LoginType.account ? password : null,

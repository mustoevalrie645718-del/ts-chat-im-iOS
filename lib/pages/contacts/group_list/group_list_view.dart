...
class GroupListViePage extends StatelessWidget {
  final logic = Get.find<GroupListLogic>();

  GroupListViePage({super.key});
      ...
      body: Column(
        children: [
          ...
          Expanded(
            child: Obx(
              () => logic.index.value == 0
                  ? _buildIVisCreatedListView()
                  : _buildIJoinedListViewWay(),
            ),
          ),
        ],
      ),
  ...
  Widget _buildIVisCreatedListView() => SmartRefresher(
  ...
  Widget _buildIJoinedListViewWay() => SmartRefresher(

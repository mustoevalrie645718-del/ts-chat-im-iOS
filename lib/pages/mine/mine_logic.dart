...
class MineLogic extends GetxController {
  ...
  Future<void> logoutHelper([bool requireLogout = true]) async {
    meetingBridge?.forceDismiss();
    rtcBridge?.dismiss();
  ...
  void _resetAppState() {
    try {
      Get.find<ConversationLogic>().clear();
  ...
  void onInit() {
    kickedOfflineSub = imLogic.onKickedOfflineSubject.listen((value) {
      ...
      } else if (value == KickoffType.userTokenExpired) {
        kickedOffline(tips: StrRes.tokenExpired);
      } else {
        kickedOffline();
      }
    });

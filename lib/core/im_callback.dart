...
mixin IMCallback {
  ...
  Function(Message msg)? onMessageDeleted;
  ...
  /// Blacklist event streams so multiple listeners can observe without overwriting callbacks
  final blacklistAddedSubject = PublishSubject<BlacklistInfo>();
  final blacklistDeletedSubject = PublishSubject<BlacklistInfo>();

  final recvNewMessageSubject = PublishSubject<Message>();
  ...
  void imSdkStatus(IMSdkStatus status, {bool reInstall = false, int? progress}) {
    Logger.print('imSdkStatus: $status, reinstall: $reInstall, progress: $progress');
  ...
  void recvNewMessage(Message msg) {
    Logger.print('recvNewMessage: ${msg.toJson()}');
    initLogic.showNotification(msg);
    recvNewMessageSubject.addSafely(msg);
  }
  ...
  void totalUnreadMsgCountChanged(int count) {
    Logger.print('totalUnreadMsgCountChanged: $count');

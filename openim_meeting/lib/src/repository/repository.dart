...
class MeetingRepository implements IMeetingRepository {
  ...
  Future<List<MeetingInfoSetting>> getUnfinished(String userID) async {
    ...
    return List<MeetingInfoSetting>.from(result['meetingDetails']
            ?.map((e) => MeetingInfoSetting()..mergeFromProto3Json(e, ignoreUnknownFields: true))
            .toList() ??
        []);
  }
  ...
  Future<LiveKit> getLiveKitToken(String meetingID, String userID) async {
    ...
    return LiveKit()..mergeFromProto3Json(result['liveKit'], ignoreUnknownFields: true);
  }
  ...
  Future<MeetingInfoSetting> getMeetingInfo(String meetingId, String userID) async {
    ...
    return MeetingInfoSetting()..mergeFromProto3Json(result['meetingDetail'], ignoreUnknownFields: true);
  }
  ...
  Future<({LiveKit? cert, MeetingInfoSetting info})> createMeeting(
    ...
    if (type == CreateMeetingType.quick) {
      ...
      final setting = CreateImmediateMeetingResp()..mergeFromProto3Json(result, ignoreUnknownFields: true);
      ...
      final setting = BookMeetingResp()..mergeFromProto3Json(result, ignoreUnknownFields: true);

      return (cert: null, info: setting.detail);
    } else {
      final result = await Apis.joinMeeting(params);
      final cert = result == null ? null : LiveKit()
        ?..mergeFromProto3Json(result, ignoreUnknownFields: true);

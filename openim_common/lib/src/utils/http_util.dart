...
class HttpUtil {
  ...
  static Future post(
    ...
    // Use cached network status to avoid delays on every request
    if (!NetworkMonitor().isAvailableSync) {
      if (showErrorToast) {
        IMViews.showToast(StrRes.networkNotStable);
      }
      throw Exception(StrRes.networkNotStable);
    }
    Logger.print('post: $path, $data');
    try {
      data ??= {};
      options ??= Options();
      options.headers ??= {};
  ...
  static Future<void> saveFileToGallery(File file, {String? name, bool showToast = true, bool isVideo = false}) async {
    Permissions.storage(() async {
      ...
      final result = isVideo
          ...
          : await ImageGallerySaverPlus.saveImage(await file.readAsBytes(), name: name);

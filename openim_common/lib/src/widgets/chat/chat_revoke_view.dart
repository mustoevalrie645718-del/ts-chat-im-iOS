...
class ChatRevokeView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (info.revokerID != null && info.revokerID == info.sourceMessageSendID) {
    ...
    final text = sender != null ? sprintf(StrRes.aRevokeBMsg, [revoker, sender]) : sprintf(StrRes.revokeMsg, [revoker]);

    text.splitMapJoin(
      RegExp('(${revoker}${sender != null ? '|$sender' : ''})'),
      onMatch: (match) {
    ...
    if (revoker == OpenIM.iMManager.userID && _isISend && canReEdit) {
      children.add(TextSpan(
        text: ' ${StrRes.reEdit}',
        style: Styles.ts_0089FF_12sp,

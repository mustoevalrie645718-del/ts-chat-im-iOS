...
class ChatItemView extends StatefulWidget {
  const ChatItemView({
    ...
    this.onTapEditMessageMenu,
    this.onTapFavoriteMenu,
    this.isPinedMenu,
    this.onTapPinedMenu,
    this.onVisibleTrulyText,
    this.onPopMenuShowChanged,
    this.onTapQuoteMessage,
    this.onTapLocateQuoteMessage,
  ...
  final Subject<MsgStreamEv<int>>? sendStatusSubject;
  ...
  final Function()? onTapEditMessageMenu;
  final VoidCallback? onTapFavoriteMenu;
  final bool Function()? isPinedMenu;
  final Function()? onTapPinedMenu;
  ...
  final Function(Message message)? onTapLocateQuoteMessage;
...
class _ChatItemViewState extends State<ChatItemView> {
  ...
  final _quotePopupController = CustomPopupMenuController();
  ...
  Widget? get _quoteMsgView {
    ...
    return quoteMsg != null
        ? ChatQuoteView(
            ...
            onTapLocate: widget.onTapLocateQuoteMessage,
            popupMenuController: _quotePopupController,
          )
        : null;
  }
  ...
  Widget? get _voiceReadStatusView => _message.isVoiceType && !IMUtils.isVoiceMessageClicked(_message.clientMsgID!)
      ? const ChatVoiceReadStatusView()
      : null;

  List<MenuInfo> get _menusItem => [
        if (widget.onTapFavoriteMenu != null)
          MenuInfo(
            icon: ImageRes.menuCollect,
            text: StrRes.collect,
            enabled: true,
            onTap: widget.onTapFavoriteMenu,
          ),
        ...
        if (widget.onTapEditMessageMenu != null)
          MenuInfo(
            icon: ImageRes.menuEdit,
            text: StrRes.edit,
            enabled: true,
            onTap: widget.onTapEditMessageMenu,
          ),
        ...
        if (widget.onTapPinedMenu != null && widget.isPinedMenu != null)
          MenuInfo(
            icon: widget.isPinedMenu!.call() ? ImageRes.menuUnpined : ImageRes.menuPined,
            text: widget.isPinedMenu!.call() ? StrRes.cancel : StrRes.top,
            enabled: true,
            onTap: widget.onTapPinedMenu,
          ),
      ];
}

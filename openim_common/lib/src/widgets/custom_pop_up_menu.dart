...
class CustomPopupMenuController extends ChangeNotifier {
  ...
  VoidCallback? _hideMenuCallback;
  ...
  // Immediately hide menu without waiting for listener
  void hideMenuImmediately() {
    menuIsShowing = false;
    _hideMenuCallback?.call();
  }
...
class _CustomPopupMenuState extends State<CopyCustomPopupMenu> {
  ...
  _showMenu() {
    // Ensure renderBoxes are available
    if (_childBox == null || _parentBox == null || !mounted) return;
  ...
  _hideMenu() {
    if (_overlayEntry != null) {
      ...
      _overlayEntry = null;
      // Force a frame to ensure overlay is completely removed
  ...
  void initState() {
    ...
    // Register immediate hide callback
    _controller?._hideMenuCallback = () {
      if (mounted) {
        _hideMenu();
      }
    };
    WidgetsBinding.instance.addPostFrameCallback((call) {
      ...
      try {
        ...
        if (renderObject is RenderBox && renderObject.attached) {
          _childBox = renderObject;
        }
        
        final overlayContext = Overlay.of(context, rootOverlay: false).context;
        ...
        if (overlayRenderObject is RenderBox && overlayRenderObject.attached) {
          _parentBox = overlayRenderObject;
        }
      } catch (e) {
        // Ignore errors if element is no longer in tree
      }
    });
  }

  @override
  void didUpdateWidget(CopyCustomPopupMenu oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (widget.controller != oldWidget.controller) {
      // Remove listener from old controller
      _controller?.removeListener(_updateView);
      _controller?._hideMenuCallback = null;
